...
### Status: 13/14 automated cases CONFIRMED PASSING
`sc016_tc002_mandatoryFieldValidation`, `sc016_tc004_fullFlowRegularToCorporate` (covers `SC_016_TC_004`+`TC_005`), `sc016_tc011_centerProgramUnchanged`, `sc016_tc017_duplicateRequestBlocked`, `sc016_tc018_customerRequestStatusAndWefDate`, `sc016_tc019_billingCancelDate`, `sc017_tc003_fullFlowCorporateToRegular` (covers `SC_017_TC_003`+`TC_004`), `sc017_tc009_centerProgramUnchanged`, `sc017_tc018_customerRequestStatusAndWefDate`, `sc017_tc020_billingCancelDate`, `sc017_tc002_duesClearanceValidation`, `bonus_migrateButtonHiddenForPausedChild`, `sc017_tc023_blockedByPendingAttritionRequest`.

- **`sc017_tc023_blockedByPendingAttritionRequest` confirmed live on child 67004** (user pre-verified before supplying it, per the caution below): the native submit confirm() DID fire this time, and the request was rejected server-side with the exact spec toast — *"OOPS! YOU CAN NOT APPLY ADMISSION MIGRATION REQUEST. YOUR ADMISSION MIGRATION REQUEST PRIOR/AFTER WITH CHILD ATTRITION REQUEST, TO PUT ADMISSION MIGRATION REQUEST THE CHILD ATTRITION REQUEST MUST BE CANCELLED."* — confirming the block happens after the confirm(), not before.

### Open items
- **`SC_017_TC_022`** (center restricted to Corporate-only admissions) still uses the `TODO_CENTER_RESTRICTED_CHILD_ID` placeholder — need a real child ID, manually pre-verified in the live UI to actually show the restriction message, before this last case can run (see the caution below — two prior candidates, 66969/66970, both turned out unrestricted and got consumed by the exploratory submit).
  - **Confirmed live: unlike the Dues check (SC_017_TC_002), this block does NOT show on modal-open** — it only surfaces at actual submit time (same as the now-confirmed attrition-conflict check), so an unconfirmed candidate risks creating a real migration request if it turns out not to be restricted.
