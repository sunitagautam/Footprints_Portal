...
### Status: 12/14 automated cases CONFIRMED PASSING (full class run, fresh children)
`sc016_tc002_mandatoryFieldValidation`, `sc016_tc004_fullFlowRegularToCorporate` (covers `SC_016_TC_004`+`TC_005`), `sc016_tc011_centerProgramUnchanged`, `sc016_tc017_duplicateRequestBlocked`, `sc016_tc018_customerRequestStatusAndWefDate`, `sc016_tc019_billingCancelDate`, `sc017_tc003_fullFlowCorporateToRegular` (covers `SC_017_TC_003`+`TC_004`), `sc017_tc009_centerProgramUnchanged`, `sc017_tc018_customerRequestStatusAndWefDate`, `sc017_tc020_billingCancelDate`, `sc017_tc002_duesClearanceValidation`, `bonus_migrateButtonHiddenForPausedChild`.

### Open items
- **`SC_017_TC_022`** (center restricted to Corporate-only admissions) and **`SC_017_TC_023`** (blocked when a Child Attrition/Withdraw request exists) still use `TODO_CENTER_RESTRICTED_CHILD_ID`/`TODO_ATTRITION_CONFLICT_CHILD_ID` placeholders — need real child IDs for each before these 2 can run.
  - **Confirmed live: unlike the Dues check (SC_017_TC_002), these two blocks do NOT show on modal-open** — both candidate children supplied so far (66969, 66970) opened the normal Add form with no restriction message, and only revealed they weren't actually restricted once a real submit was attempted (both now have their own genuine Pending Admission Migration request as a side effect — consumed, not center-restricted/attrition-conflicted).
  - **Caution for next candidates**: because the block apparently only surfaces at actual submit time (not on open) for these two scenarios, any exploratory test on an unconfirmed child risks creating a real migration request if it turns out NOT to be restricted — same risk just hit twice. Recommend the child be manually pre-verified (in the live UI) to actually show the blocking behavior before handing it over, rather than supplying an untested candidate.
- Fresh Regular + Corporate children needed for the next clean full-suite re-run (both consumed by the confirmed-passing run above) — backups **71341** (Regular) and **66865** (Corporate) were supplied but not yet used.
